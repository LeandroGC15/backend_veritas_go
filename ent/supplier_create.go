...
func (sc *SupplierCreate) SetRucNit(s string) *SupplierCreate {
	sc.mutation.SetRucNit(s)
	return sc
}

// SetNillableRucNit sets the "ruc_nit" field if the given value is not nil.
func (sc *SupplierCreate) SetNillableRucNit(s *string) *SupplierCreate {
	if s != nil {
		sc.SetRucNit(*s)
	}
	return sc
...
func (sc *SupplierCreate) SetTenantID(i int) *SupplierCreate {
...
func (sc *SupplierCreate) AddPurchaseInvoiceIDs(ids ...int) *SupplierCreate {
	sc.mutation.AddPurchaseInvoiceIDs(ids...)
	return sc
}

// AddPurchaseInvoices adds the "purchase_invoices" edges to the PurchaseInvoice entity.
func (sc *SupplierCreate) AddPurchaseInvoices(p ...*PurchaseInvoice) *SupplierCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return sc.AddPurchaseInvoiceIDs(ids...)
}
...
func (sc *SupplierCreate) createSpec() (*Supplier, *sqlgraph.CreateSpec) {
	...
	if value, ok := sc.mutation.RucNit(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
	...
	if nodes := sc.mutation.PurchaseInvoicesIDs(); len(nodes) > 0 {
